...
pub async fn list_predicates(State(s): State<DbState>) -> Res {
    let rows = store::list_predicates(&s.db).await.map_err(e500)?;
    Ok(Json(json!({ "predicates": rows })))
}

pub async fn create_predicate(State(s): State<DbState>, Json(input): Json<CreatePredicate>) -> Res {
    let row = store::create_predicate(&s.db, input).await.map_err(e500)?;
    Ok(Json(json!({ "predicate": row })))
}

pub async fn delete_predicate(State(s): State<DbState>, Path(id): Path<i64>) -> Res {
...
pub async fn list_entities(State(s): State<DbState>) -> Res {
    let rows = store::list_entities(&s.db).await.map_err(e500)?;
    Ok(Json(json!({ "entities": rows })))
}

pub async fn create_entity(State(s): State<DbState>, Json(input): Json<CreateEntity>) -> Res {
    let row = store::create_entity(&s.db, input).await.map_err(e500)?;
    Ok(Json(json!({ "entity": row })))
}

pub async fn delete_entity(State(s): State<DbState>, Path(id): Path<i64>) -> Res {
...
pub async fn list_rules(State(s): State<DbState>) -> Res {
...
pub async fn delete_rule(State(s): State<DbState>, Path(id): Path<i64>) -> Res {
...
pub async fn list_sentences(State(s): State<DbState>) -> Res {
    let rows = store::list_sentences(&s.db).await.map_err(e500)?;
    Ok(Json(json!({ "sentences": rows })))
}

pub async fn create_sentence(State(s): State<DbState>, Json(input): Json<CreateSentence>) -> Res {
    let row = store::create_sentence(&s.db, input).await.map_err(e500)?;
    Ok(Json(json!({ "sentence": row })))
}

pub async fn delete_sentence(State(s): State<DbState>, Path(id): Path<i64>) -> Res {
