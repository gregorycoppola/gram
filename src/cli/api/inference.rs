...
use crate::core::qbbn::factor_graph::{CPTTable, GraphEdge, GraphSnapshot};
...
fn print_inference_pretty(result: &InferenceResult) {
    ...
    let (proposition_count, group_count, and_count, or_count, _negation_count, evidence_count) =
        result.stats;

    println!(
        "  graph: {} propositions, {} groups, {} AND, {} OR, {} evidence",
        proposition_count, group_count, and_count, or_count, evidence_count
    );

    let topology_policy = if result.topology.bp_should_be_exact() {
        "BP must equal exact"
    } else {
    ...
    if let Some(partition) = result.exact_partition_function {
        println!("  exact partition function: {:.8}", partition);
    }
    ...
    for query in &result.query_results {
        ...
        println!(
            "  {} P({})  BP={:.6}{}{}{}",
            status, query.formula, query.prob, exact, delta, expected
        );
    }
...
fn print_graph(graph: &GraphSnapshot) {
    ...
    let mut downstream_and: HashMap<&str, Vec<&str>> = HashMap::new();

    for edge in graph.edges.iter().filter(|edge| edge.edge_type == "and") {
    ...
    let source_propositions: Vec<_> = graph
        .nodes
        .iter()
        .filter(|node| node.node_type == "proposition" && !or_targets.contains(node.id.as_str()))
        .collect();
    ...
    for node in &source_propositions {
        ...
        let probability = if node.is_evidence {
            format!("evidence={:.4}", node.evidence_prob.unwrap_or(0.5))
        } else {
            format!("belief={:.4}", node.belief)
        };

        let downstream = format_downstream_and(&downstream_and, node.id.as_str());

        println!(
            "  {} {}  {:<32}  {}{}",
            kind, node.id, formula, probability, downstream
        );
    }
    ...
    for node in &groups {
        let polarity = if node.negated { "negative" } else { "positive" };

        let and_factor = and_by_target.get(node.id.as_str()).copied().unwrap_or("?");

        let conclusion = node.conclusion_id.as_deref().unwrap_or("—");
        ...
        println!(
            ...
            node.id, polarity, and_factor, rule, conclusion, or_factor
        );
    }
    ...
    for node in &computed_propositions {
        ...
        let or_factor = or_by_target.get(node.id.as_str()).copied().unwrap_or("?");

        let downstream = format_downstream_and(&downstream_and, node.id.as_str());

        println!(
            "  {}  {:<32}  or={:<6}  belief={:.4}{}",
            node.id, formula, or_factor, node.belief, downstream
        );
    }
    ...
    for edge in graph.edges.iter().filter(|edge| edge.edge_type == "and") {
        print_rule_flow(graph, edge, &or_by_target);
    }

    println!("\n  ── factors ──");

    for edge in &graph.edges {
        let inputs = display_edge_inputs(graph, edge);
...
fn print_rule_flow(graph: &GraphSnapshot, edge: &GraphEdge, or_by_target: &HashMap<&str, &str>) {
    let input_literals: Vec<String> = edge
        .source_ids
        .iter()
        .enumerate()
        .map(|(index, source_id)| {
            let node = graph.nodes.iter().find(|node| node.id == *source_id);

            let belief = node.map(|node| node.belief).unwrap_or(0.5);

            let negated = edge.input_negated.get(index).copied().unwrap_or(false);

            let literal_probability = if negated { 1.0 - belief } else { belief };

            if negated {
                format!("¬{}({:.4})", source_id, literal_probability)
            } else {
                format!("{}({:.4})", source_id, literal_probability)
            }
        })
    ...
    let group = graph.nodes.iter().find(|node| node.id == edge.target_id);
    ...
    let conclusion = group.conclusion_id.as_deref().unwrap_or("?");

    let or_factor = or_by_target.get(conclusion).copied().unwrap_or("?");
    ...
    let weight = group
        ...
        .and_then(|rule_id| {
            graph
                .rules
                .iter()
                .find(|rule| rule.id.as_str() == rule_id.as_str())
        })
...
fn print_cpt(graph: &GraphSnapshot, cpt: &CPTTable) {
    let edge = graph.edges.iter().find(|edge| edge.id == cpt.factor_id);
    ...
    let display_inputs: Vec<String> = cpt
        ...
        .map(|(index, input)| {
            let negated = edge
                .and_then(|edge| edge.input_negated.get(index))
                .copied()
                .unwrap_or(false);
    ...
    if cpt.truncated {
        ...
        let header = display_inputs.join(" | ");

        println!("    {:<24} | P(out=1)", header);

        println!("    {}", "─".repeat(header.len().max(24) + 13));
        ...
        for row in &cpt.rows {
            ...
            println!("    {:<24} | {:.4}", values.join(" | "), row.prob_true);
        }
    }
...
fn display_edge_inputs(graph: &GraphSnapshot, edge: &GraphEdge) -> Vec<String> {
    edge.source_ids
        .iter()
        .enumerate()
        .map(|(index, source_id)| {
...
fn format_downstream_and(downstream_and: &HashMap<&str, Vec<&str>>, node_id: &str) -> String {
    downstream_and
        .get(node_id)
        .map(|factor_ids| format!(" → {}", factor_ids.join(", ")))
        .unwrap_or_default()
}
